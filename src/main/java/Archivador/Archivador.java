...
public class Archivador {
	...
	public void eliminarPedidoArchivotxt(String codigo, String archivo, JPanel mainPanel) {
		// Leer el contenido del archivo
		List<String> lineas = new ArrayList<>();
		...
		for (int i = 0; i < lineas.size(); i++) {
			String linea = lineas.get(i);
			if (linea.startsWith("Patente: ") && linea.contains(codigo)) {
		...
		try (PrintWriter writer = new PrintWriter(archivo)) {
			...
			JOptionPane.showMessageDialog(mainPanel, "Código eliminado del archivo: " + codigo);
		} catch (FileNotFoundException e) {
			JOptionPane.showMessageDialog(mainPanel, "Error al guardar los cambios en el archivo: " + e.getMessage());
		}
	}
